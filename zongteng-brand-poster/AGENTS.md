...
- Never produce PPT-style poster layouts. Avoid centered titles with equal cards, pale dashboard grids, generic icon circles, and slide-like information blocks. Read `references/design-quality-bar.md` and design a poster with a strong hero visual, visual hierarchy, texture/motion/layering, and campaign-level impact.
...
1. Run guided mode by default for a new poster request. Ask the brief questions from `references/guided-brief.md` and wait for the user before creating final files.
...
3. Read `references/style-index.json`, `references/design-quality-bar.md`, and the reference files needed for the selected poster type.
...
6. Check brand fit and design quality before delivery: logo integrity, color palette, value/IP mapping, legibility, spacing, target format, real-text Chinese rendering, and no PPT-like layout.
...
After asking the brief, stop and wait for the user's answer unless they explicitly asked you to decide and generate immediately.
...
- Use a campaign-poster composition with a dominant hero visual, not a card grid.
- Add one culture/IP or brand-energy element when the topic is people, culture, recognition, values, or employee engagement.
